  ...
  <v-layout>
    ...
    >
      ...
      <nav class="longNav" v-if="!isLogged">
        ...
        <RouterLink class="aboutLink" to="/sobre" name="">Sobre Eco-Escolas</RouterLink>
        <RouterLink class="faqLink" to="/faq" name="">F.A.Q.</RouterLink>
        <button id="login" class="navbtn btnP">
          <RouterLink v-on:click="ToHide" to="/login" name="">Entrar</RouterLink>
        </button>
      ...
      <nav class="longNav" v-if="isLogged && this.user.type == 'user'">
        <RouterLink class="homeLink" to="/Home" name="">Página principal</RouterLink>
        <RouterLink class="actLink" to="/Activities" name="">Atividades</RouterLink>
        <RouterLink class="ocLink" to="/Occurrence" name="">Ocorrências</RouterLink>
        <RouterLink class="formLink" to="/Form" name="">Questionário</RouterLink>
...
export default{setup(){const e=useUsersStore();
return{usersStore:e,Logged:e.getLoggedObj,missionStore:useMissionStore()}},created(){this.users=this.usersStore.getUsers},name:"App",data(){return{users:this.usersStore.getUsers,user:this.usersStore.getUsers.find((e=>e.email==this.usersStore.getLogged)),drawer:!1,group:null,collapse:!1,tohide:"",newAdmin:{type:"",name:"",email:"",school:"",username:"",password:"",passConf:""},dialogAdd:!1}},updated(){},computed:{isLogged(){return!!this.usersStore.getLogged},ToHide(){return"login"!=this.$route.name&&"signUp"!=this.$route.name},getUser(){this.users=this.usersStore.getUsers,this.user=this.usersStore.getUsers.find((e=>e.email==this.usersStore.getLogged))},updateNotifs(){let e=[],s=this.missionStore.getMissions;return s.forEach((s=>{s.users.forEach((t=>{"Concluída"!=t[2]||this.user.rewards.includes(s.reward)||t[0]!=this.user.email||e.push(s)}))})),e.length>0}},methods:{logOut(){this.usersStore.logOut(),this.$router.push("/")},onSubmit(){-1!=this.newAdmin.password.indexOf(" ")?alert("Palavra-passe não pode conter espaços."):this.newAdmin.password.length<8?alert("Palavra-passe tem de ter no mínimo 8 caracteres."):-1!=this.newAdmin.username.indexOf(" ")?alert("Nome de utilizador não pode conter espaços."):this.newAdmin.username.length<3?alert("Nome de utilizador tem de ter no mínimo 3 caracteres."):this.newAdmin.type&&this.newAdmin.name&&this.newAdmin.email&&this.newAdmin.username&&this.newAdmin.password&&this.newAdmin.passConf&&(this.dialogAdd=!1,this.usersStore.newAdmin(this.newAdmin.type,this.newAdmin.name,this.newAdmin.email,this.newAdmin.username,this.newAdmin.password,this.newAdmin.passConf),this.newAdmin.type="",this.newAdmin.name="",this.newAdmin.email="",this.newAdmin.username="",this.newAdmin.password="",this.newAdmin.passConf="")}}};
</script>
