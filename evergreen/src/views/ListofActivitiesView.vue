<template>
  <v-dialog v-model="open">
    <div class="fieldP modal">
      <div>
        <button
          class="btnRound btnP"
          @click="(open = false), (this.listUsers = [])"
        >
  ...
  >
    <div>
      <button class="btnRound btnP" @click="listDialog = false">
        <img
          style="width: 15px"
          src="../assets/icons/icones/close.svg"
        /></button
      ><br /><br />
    </div>
  ...
  <div class="listAct" style="padding: 30px">
    <div class="grid-item" v-for="activity in activities">
      <v-card class="max-auto cardPc" max-width="400">
        <v-img
          class="image cardImg"
          :src="activity.photo"
          height="219"
...
export default {
  ...
  methods: {
    async Participation() {
      ...
      this.activity = await this.activityStore.verifyParticipation(
        this.activity,
        selUser
      );
      ...
      await this.activityStore.fetchCoordinatorActivities();
      await this.missionStore.getAllMissions();
      await this.missionStore.completeMission(this.user.user, 1);
      await this.missionStore.completeMission(this.user.user, 3);
    },
  },
